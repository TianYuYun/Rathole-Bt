...
class rathole_manager_main:
    ...
    def _render_config(self, mode, base_addr, services, transport='noise', noise_key=''):
        lines = ['# Generated by BT Rathole Manager 2.2.', '# Edit from the BT panel to keep state synchronized.', '']
        if mode == 'server':
            lines += [
                '[server]',
    ...
    def get_plugin_update(self, args):
        try:
            meta = self._github_json(self.UPDATE_URL)
        ...
        try:
            ...
            for name in files:
                ...
                if not re.fullmatch(r'[A-Za-z0-9_.-]{1,128}', name):
                    raise RuntimeError('更新文件名无效：%s' % name)
            ...
            with tempfile.TemporaryDirectory(prefix='rathole-plugin-update-') as temp_dir:
                for name in safe_files:
                    ...
                    with urllib.request.urlopen(req, timeout=30) as response, open(os.path.join(temp_dir, name), 'wb') as output:
                        shutil.copyfileobj(response, output)

                py_file = os.path.join(temp_dir, 'rathole_manager_main.py')
                ...
                if os.path.isfile(info_file):
                    ...
                    if str(info.get('versions', '')).strip() != latest:
                        raise RuntimeError('新版 info.json 版本号与更新清单不一致')

                install_file = os.path.join(temp_dir, 'install.sh')
